...
class Adapter_Spectral(nn.Module):
    ...
    def __init__(self, NUM_TOKENS=None, img_size=224, in_chans=3, embed_dim=768, depth=12,
        ...
        #self.freeze_attn()
        self.conv_head = nn.Conv2d(in_chans, in_chans, kernel_size=3, padding=1) #TODO: REMEMBER CHANNELS BJORNOLAV #TODO: TEST IF ORIGINA CHANNLES WORKS
    ...
    def forward_features(self, x, deform_inputs, H, W):
    ...
    def forward(self, x, deform_inputs, H, W):
        # x = self.conv_head(x)
        # x = self.conv1_reconstruct(x)

        x = self.forward_features(x, deform_inputs, H, W)
        x = x[-1]
