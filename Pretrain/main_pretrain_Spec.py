...
import models_mae_Spec
import sys

from engine_pretrain import train_one_epoch

from util.datasets import HyperionDataset
...
def get_args_parser():
    ...
    parser.add_argument('--batch_size', default=int(256*3), type=int, help='Batch size per GPU (effective batch size is batch_size * accum_iter * # gpus')
    parser.add_argument('--epochs', default=1600, type=int)
    parser.add_argument('--accum_iter', default=1, type=int, help='Accumulate gradient iterations (for increasing the effective batch size under memory constraints)')

    # Model parameters
    parser.add_argument('--model', default='spec_mae_b', type=str, metavar='MODEL', help='Name of model to train')

    parser.add_argument('--image_size', default=64, type=int, help='images input size')

    parser.add_argument('--mask_ratio', default=0.25, type=float, help='Masking ratio (percentage of removed patches).')
    ...
    # Dataset parameters
    parser.add_argument('--data_path', default='/home/sigma_pub1/GF5_Crop', type=str, help='dataset path')
    parser.add_argument('--in_chans', default=100, type=int, help='input channels')
    parser.add_argument('--num_tokens', default=100, type=int, help='generate tokens in spec MAE')
...
def main(args):
    ...
    if 'spec' in args.model:
        print('MAE pretraining ViT series model')
        model = models_mae_Spec.__dict__[args.model](args, inchannels=args.in_chans)  #
    else:
