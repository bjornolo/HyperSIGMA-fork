...
class SpatialVisionTransformer(nn.Module):
    ...
    def forward(self, x):
        # x = self.conv_head(x) # torch.Size([8, 100, 64, 64])
        # print(x.shape)
        # exit()
        ...
        # print(x1.shape), print(x0.shape)
        # x1 = self.up1(x1)
        # x1 = self.up2(x1)
        # x1 = torch.nn.functional.interpolate(x1, scale_factor=2, mode='bilinear', align_corners=True)
