...
class SpatViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=80, embed_dim=768, depth=12,
                 ...
                 out_indices=[11], interval=3, pretrained=None, restart_regression=True, n_points=8, original_channels=191, scale=4):
        super().__init__()
        self.patch_size = patch_size
        ...
        self.conv1_reconstruct = nn.Conv2d(180, 180, kernel_size=3, padding=1)
        ...
        self.upsample = Upsampler(default_conv, scale, in_chans)
    ...
    def forward(self, x, lms):
        ...
        x = torch.cat([x0, x], dim=1) # torch.Size([8, 200, 64, 64])
        x = self.conv3_reconstruct(x) # torch.Size([8, 100, 64, 64])
        x = self.upsample(x)
